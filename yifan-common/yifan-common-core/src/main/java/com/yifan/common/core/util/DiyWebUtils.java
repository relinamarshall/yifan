...
import com.yifan.common.core.exception.CheckedException;
 ...
 *
 * @author Wenzhou
 * @since 2023/5/5 20:43
 */
@Slf4j
@UtilityClass
public class DiyWebUtils extends WebUtils {
    /**
     * BASIC_
     */
    ...
    public boolean isBody(HandlerMethod handlerMethod) {
        ResponseBody responseBody = DiyClassUtils.getAnnotation(handlerMethod, ResponseBody.class);
    ...
    public String getCookieVal(String name) {
        if (DiyWebUtils.getRequest().isPresent()) {
            return getCookieVal(DiyWebUtils.getRequest().get(), name);
        }
        return null;
    }
    ...
    public String getClientId() {
        if (DiyWebUtils.getRequest().isPresent()) {
            String header = DiyWebUtils.getRequest().get().getHeader(HttpHeaders.AUTHORIZATION);
            return splitClient(header)[0];
        }
        return null;
    }
