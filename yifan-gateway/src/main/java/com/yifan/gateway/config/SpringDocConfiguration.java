...
public class SpringDocConfiguration {
    ...
    @Bean
    @Lazy(false)
    @ConditionalOnProperty(name = "springdoc.api-doc.enabled", matchIfMissing = true)
    public List<GroupedOpenApi> apis(SwaggerUiConfigParameters swaggerUiConfigParameters,
                                     SwaggerDocProperties swaggerDocProperties) {
        List<GroupedOpenApi> groups = new ArrayList<>();
        for (String value : swaggerDocProperties.getServices().values()) {
